...
void do_bgfg(char **argv) {
	...
	if (argv[1]) {
		...
		if (job) {
			...
			if (id != 0) {
				if (!strncmp(argv[1], "%", 1)) { printf("%s: No such job\n", argv[1]); }
				else { printf("(%s): No such process\n", argv[1]); }
			}
			else {
				if (!strcmp(argv[0], "bg")) { printf("bg: argument must be a PID or %sjobid\n", "%"); }
				else { printf("fg: argument must be a PID or %sjobid\n", "%");	}
			}
		}
	}
	else {
		if (!strcmp(argv[0], "bg")) { printf("bg command requires PID or %sjobid argument\n", "%"); }
		else { printf("fg command requires PID or %sjobid argument\n", "%"); }
	}
