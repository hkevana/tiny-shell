...
void eval(char *cmdline) 
{
	...
	int cmds[MAXARGS];
	int in_redir[MAXARGS];
	int out_redir[MAXARGS];

	// parseline returns bg --> unused for shell lab 1
	// int bg;
	parseline(cmdline, args);
	int argc = parseargs(args, cmds, in_redir, out_redir);
	...
	if (!builtin_cmd(args)) {
		...
		for (i = 0; i < argc; i++) {
			...
			if ((pid = fork()) > 0) {
				...
				if (argc > 1) {
					if (i != 0) {
						if (close(prevfd) == -1) { printf("%d: Error in parent closing prev [%d]: %s\n", i, prevfd, strerror(errno)); }
					}
					if (i < argc - 1) {
						prevfd = pipefd[READ];
						if (close(pipefd[WRITE]) == -1) { printf("%d: Error in parent closing pipe WRITE [%d]: %s\n", i, pipefd[1], strerror(errno)); }
					}
				}

				// Group work
				all_pids[i] = pid;
				setpgid(pid, all_pids[0]);
			}
			else if (pid == 0) {
				int in = in_redir[i];
				int out = out_redir[i];
				...
				if (argc > 1) {
					// First - Dup WRITE
					if (i == 0) { dup2(pipefd[WRITE], 1); }
					// Last - Dup READ
					else if (i == (argc - 1)) { dup2(prevfd, 0); }
					// Middle - Dup BOTH
					else {
						dup2(prevfd, 0);
						dup2(pipefd[WRITE], 1);
					...
					// Close pipes
					if (i < argc - 1) {
						if (close(pipefd[READ]) == -1) { printf("%d: Error in child closing pipe READ [%d]: %s\n", i, pipefd[0], strerror(errno)); }
						if (close(pipefd[WRITE]) == -1) { printf("%d: Error in child closing pipe WRITE [%d]: %s\n", i, pipefd[1], strerror(errno)); }
					}
					if (i != 0) {
						if (close(prevfd) == -1) { printf("%d: Error in child closing prev [%d]: %s\n", i, prevfd, strerror(errno)); }
					}
				}
				// Execute program
				if (execve(args[cmds[i]], &args[cmds[i]], environ) == -1) {
